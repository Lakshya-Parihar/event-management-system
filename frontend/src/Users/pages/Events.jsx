...
const Events = () => {
  ...
  //   Featured Events Declaration part
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost/EMS/backend/get_events.php", {
        status: "publish",
      })
      .then((res) => setFeaturedEvents(res.data))
      .catch((err) => console.error("Error fetching featured events:", err));
  ...
  //   Featured Events Part End

  // Upcoming Events declaration part
    ...
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        <input
          ...
          className="mb-8 px-4 py-2 w-50 border border-gray-300 rounded-lg"
        />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {filteredFeaturedEvents.map((event) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6 }}
                className="w-80 h-[420px] bg-white rounded-2xl shadow-xl shadow-gray-300 hover:shadow-blue-200 p-4 flex flex-col justify-between items-center"
              >
                <img
                  src={event.image}
                  alt={event.title}
                ...
                <button
                  ...
                  className="spiral-hover pop-hover text-black mt-3 px-3 py-2 rounded-xl shadow-xl font-semibold bg-gradient-to-r from-blue-500 to-green-400"
                >
                  View Details
                </button>
              </motion.div>
      ...
      {/* Upcoming Events  */}
      <section className="overflow-hidden bg-white py-24 px-8 min-h-[350px]">
        <motion.h2
        ...
        <div className="relative w-full overflow-hidden">
          ...
          >
            {[...upcomingEvents, ...upcomingEvents].map((event, index) => (
              <motion.div
                key={`${event.id}-${index}`}
                className="mb-10 min-w-[280px] max-w-[400px] h-[150px] bg-white/30 backdrop-blur-lg border border-white/40 rounded-2xl shadow-xl flex items-center px-4 py-3 relative transition-all duration-500"
              ...
              >
                ...
                <div className="flex-1 px-4 text-left">
                  <h3 className="text-md font-bold text-indigo-800 mb-1 leading-tight">
                    {event.title}
                  </h3>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="mt-3 bg-blue-500 text-white px-3 py-1 rounded-full text-xs shadow hover:bg-blue-600"
                  >
                    Details
                  </motion.button>
                </div>
  ...
  `}</style>
      ...
      <footer className="text-white">
        <motion.div
          className="backdrop-blur-md bg-gradient-to-r from-blue-600/60 to-cyan-500/60 px-8 py-4 rounded-t-2xl shadow-lg sticky top-0 z-9999 flex justify-between items-center"
          initial={{ y: -80, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.6, type: "spring", stiffness: 80 }}
        >
